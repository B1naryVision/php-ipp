...
class http_class
{
    ...
    private $headers;
    private $reply_headers = []; // time waiting for connection, seconds
    private $reply_body = []; // time waiting for data, milliseconds
    private $connection; // time waiting between data chunks, millisecond
    private $arguments;
    private $last_limit;
    private $connected;
    private $nc = 1;
    ...
    public $debug;
    public $html_debug; // chunk size of data
    public $timeout = 30; // compatibility mode for old scripts
    ...
    public function SendRequest($arguments)
    {
        ...
        $result = $this->_StreamRequest($arguments);
        if (!$result[0]) {
        ...
        if (!in_array('www-authenticate', $headers, true)) {
        ...
        switch ($authtype) {
            ...
            case 'digest':
                $arguments['Headers']['Authorization'] = $this->_BuildDigest();
                break;
    ...
    private function _StreamRequest($arguments): array
    {
        $this->status = false;
        $this->reply_headers = [];
        $this->reply_body = '';
        ...
        foreach ($this->arguments['BodyStream'] as $argument) {
            ...
            if ($type == 'Data') {
                ...
                if (is_readable($value)) {
                    $file = fopen($value, 'rb');
                    while (!feof($file)) {
                        if (@fread($file, $this->window_size)
                        ) {
    ...
    private function _HttpError($msg, $level, $errno = null): array
    {
        $trace = '';
        $backtrace = debug_backtrace();
        foreach ($backtrace as $trace) {
            $trace .= sprintf("in [file: '%s'][function: '%s'][line: %s];\n", $trace['file'], $trace['function'],
                $trace['line']);
        }
        $msg = sprintf('%s\n%s: [errno: %s]: %s',
    ...
    private function _streamString($string): bool
    {
    ...
    private function _ReadStream(): bool
    {
    ...
    private function _BuildDigest(): string
    {
